...
LEGACY_METADATA_SHA256 = "93093645f4c80a5070a1bba294b7a3171ac006174e552b572346c21c3fd1fea3"
SCHEMA_VERSION = 3
...
OPERATION_BUNDLE_SHA256 = "0283e0990d9116cc382624223a89a6e6442125a3357f03c77468f25c13c147d6"
...
TRANSPORTS = {"public_http": 78, "rest": 79, "custom_container": 7, "graphql": 6, "internal_binding": 1, "mcp": 1}
ACCESS = {"raw_rest": 126, "modeled": 7, "mcp_remote": 26, "public_direct": 6, "raw_graphql": 6, "blocked": 1}
...
def apply_schema_bundle(catalog):
    ...
    for row in catalog["capabilities"]:
        ...
        if row["name"] in operation_by_name:
            ...
            browser = row["name"] in {"get_url_links", "get_url_markdown", "scrape_url_elements", "get_url_json", "get_url_snapshot", "get_crawl_result", "list_browser_sessions"}
...
def validate_operation_contracts(catalog, operations, rows, evidence):
    ...
    if operations["version"] != "phase4c-operation-contracts-v1" or operations["source_commit"] != COMMIT:
    ...
    for index, contract in enumerate(contracts):
        ...
        def exact_discovery(item):
            provenance = item["provenance"]
            browser_discovery = {"get_url_links", "get_url_markdown", "scrape_url_elements", "get_url_json", "get_url_snapshot", "get_crawl_result", "list_browser_sessions"}
            return (
...
def validate(catalog):
    ...
    if operations.get("version") != "phase4c-operation-contracts-v1" or json_sha256({**operations, "bundle_sha256": None}) != OPERATION_BUNDLE_SHA256 or operations.get("bundle_sha256") != OPERATION_BUNDLE_SHA256 or operations.get("contract_count") != len(contracts) or contracts != sorted(contracts, key=lambda item: item.get("capability", "")) or len(contracts) != 16:
    ...
    if sum("path_template" in row for row in rows) != 13:
