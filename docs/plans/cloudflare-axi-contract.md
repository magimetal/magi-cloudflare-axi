...
Phase 4F authenticated `auditlogs_by_account_id` remains available at `GET /accounts/{account_id}/logs/audit`; invoke `magi-cloudflare-axi capability invoke auditlogs_by_account_id --input '{"since":"<since>","before":"<before>"}' --allow-egress`. Phase 4G adds authenticated AutoRAG `list_rags`. Exact contracts now total 21, with 16 discovery-verified. Invoke AutoRAG with `magi-cloudflare-axi capability invoke list_rags --input '{}' --allow-egress`. Pinned app-specific OAuth scope additions are `account:read` and `rag:write`; no API-token permission or least-privilege claim is inferred. Route is `GET /accounts/{account_id}/autorag/rags` with ordered `page`, `per_page` query parameters, defaults 1 and 20, bounds `page >= 1` and `per_page` 1–50, transient GET retries, redirect refusal, full response validation before strict `autorags` projection, exact numeric `total_count` preservation, and an 8 MiB failure bound. Deprecated upstream AutoRAG server context does not imply operation unavailability. Current vector is `I=172; S=172; R=B=P=V=21; D=16; X=40`; 151 routes remain unresolved.
