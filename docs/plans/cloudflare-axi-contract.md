...
Capability contract: catalog schema v3 proves 172 registered names and 172 canonical registration-input contracts. Phase 4B adds three authenticated Browser reads—`get_url_json`, `get_url_snapshot`, and `get_crawl_result`—for 15 exact contracts total; six Browser reads are discovery-verified through shared hermetic discovery integration test. Vector: `I=172; S=172; R=B=P=V=15; D=10; X=40`; 157 routes unresolved. Phase 4 remains in progress. Pinned source `/browser-run` is route authority; official `/browser-rendering` docs corroborate semantics only, and route/transport mismatch remains explicit.
