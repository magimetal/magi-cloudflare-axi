...
Phase 1 foundation evidence (active, 2026-08-10): `tools/schema-extractor` uses pinned Git blob reads, exact checkout `HEAD` and tree verification, embedded canonical catalog, and Oxc 0.75.1 typed TypeScript AST with lexical symbol resolution. Census scope is exact `apps/*/(src|server)/**/*.ts` plus `packages/mcp-common/src/**/*.ts`, excludes specs, and gates 114 files / 172 records. Current CI asserts deterministic repeated output, observed root-kind counts (`input_schema=139`, `dex_raw_shape=18`, `casb_params=11`, `implicit_zero_input=4`), and typed expression-root counts (`zod_object_call=139`, `object_shape=29`, `zero_input=4`). Per-record syntax features inventory only nodes inside exact schema spans; it does not claim shared-binding resolution or canonical JSON Schema support. No schema semantic `resolved` claim, catalog status changes, provider calls, or credentials. Phase 1 tasks and exit gates remain unchecked; S remains 0.
