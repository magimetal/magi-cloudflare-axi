...
Phase 1 foundation evidence (active, 2026-08-10): `tools/schema-extractor` uses pinned Git blob reads, exact checkout `HEAD` and tree verification, embedded canonical catalog, and Oxc 0.75.1 typed TypeScript AST with lexical symbol resolution. Census scope is exact `apps/*/(src|server)/**/*.ts` plus `packages/mcp-common/src/**/*.ts`, excludes specs, and gates 114 files / 172 records. Current CI asserts deterministic repeated output, observed root-kind counts (`input_schema=139`, `dex_raw_shape=18`, `casb_params=11`, `implicit_zero_input=4`), and typed expression-root counts (`zod_object_call=139`, `object_shape=29`, `zero_input=4`). Per-record syntax features inventory only nodes inside exact schema spans. Direct lexical binding provenance records same-file initializer spans, expressions, hashes, and import source/form metadata. Every observed direct binding has an exact target: 408 named imports resolve to initialized exported values in 14 pinned internal Git blobs, and 48 same-file initialized values retain exact local evidence. Recursive value-dependency census now closes all 456 direct-binding roots over 217 unique pinned declarations in 20 files, 269 deterministic unique dependency edges, and 642 root-to-leaf resolution chains with maximum depth 4. Closure evidence includes exact source/span/blob/hash metadata for 213 variable initializers and 4 function declarations; 208 exact value-source hashes are distinct. Typed lexical analysis filters type-only and shadowed-local references. Boundaries remain explicit, occurrence-specific, and unexecuted: 18 external Zod import declarations, 17 language-built-in references, and 1 registration-context parameter, each tied to exact pinned file/blob/span evidence. Cycles, cache-safe depth or chain overflow, boundary metadata conflicts, named re-exports, unsupported internal import forms, unresolved globals, unsupported local declarations, ambiguity, path escape, invalid blobs/UTF-8, and parser/semantic diagnostics fail closed; current pinned closure has zero unsupported dependency constructs. This remains source provenance only: no shared-binding semantic evaluation or canonical JSON Schema claim. No schema semantic `resolved` claim, catalog status changes, provider calls, or credentials. Phase 1 tasks and exit gates remain unchecked; catalog remains unchanged and S remains 0.
