...
Phase 4D adds authenticated `get_url_pdf` and `get_url_screenshot`, bringing exact contracts to 18. Phase 4 cohort now has nine discovery-verified authenticated Browser reads; with Phase 2's generated `get_url_markdown`, Browser family has ten exact contracts. Vector `I=172; S=172; R=B=P=V=18; D=13; X=40`; discovery is 13 verified and five generated, with 154 routes unresolved. PDF sends `POST /accounts/{account_id}/browser-run/pdf` with `{url}` and accepts only `application/pdf` plus `%PDF-`; screenshot sends `POST /accounts/{account_id}/browser-run/screenshot` with `{url,viewport}` and accepts only `image/png` plus PNG signature. Screenshot omits absent viewport because JSON cannot represent JavaScript `undefined`; supplied viewport defaults nested width/height to 800×600 and strips unknown nested keys. Both require explicit new filesystem `--output`, refuse overwrite, prepare a private sibling temporary file before auth/network, enforce an 8 MiB response limit, never retry or follow redirects, and return path, MIME type, byte count, and SHA-256 metadata. Pinned `GET /accounts/{account_id}/browser-run/devtools/session` remains route authority for `list_browser_sessions`; official docs instead show `/browser-rendering/devtools/session` with optional `limit`/`offset`, while pinned zero-input handler exposes neither query. Phase 4 remains in progress.
