...
`capability list/get` exposes compact evidence from 172 registered tool names at pinned source commit `70ff690553722f731849ede6ba9ce98958395a23`. This is **inventory parity only**, not proof that every catalog schema or guessed REST mapping is complete. `tool list --server <server>` and `tool schema` query live authoritative MCP schemas. `tool list --all` reveals full local metadata for audit work.
