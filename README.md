...
Catalog schema v3 and Phase 3 governance are canonical.
`capability list/get` exposes 172 registered tool names at pinned source commit `70ff690553722f731849ede6ba9ce98958395a23`. Phase 3 completes proven direct unauthenticated Blog cohort: `I=172; S=172; R=B=P=V=9; D=4; X=40`; four Blog operations are discovery-verified, five prior operations discovery-generated. Blog routes use public HTTP; `search_posts` is corrected to `POST /search`, other Blog operations are GET. Remaining 163 routes unresolved. Research pool correction: initial 80 legacy `public_http` reads = Browser 9 + Radar 65 + Blog 4 + demo 1 + stack 1. Browser/Radar require authoritative route, transport, scope, and authentication reclassification during Phase 4 research; demo/stack require source-hosted or MCP route research during Phase 5. DNS Analytics is outside this legacy pool. No live calls required.
...
Phase 0–3 governance artifacts: `capabilities/cloudflare-mcp-parity.json` is canonical; fixed per-name legacy-metadata digest protects migration integrity. Schema v3 adds typed evidence provenance plus a hash-bound operation-contract artifact. Repository-development commands `scripts/catalog-governance.py validate`, `self-test`, `generate`, `metrics`, `report`, `sync-schemas`, and `check` validate the catalog, run negative mutations, regenerate deterministic artifacts, print them, and detect drift. CI rejects stale artifacts, source drift, duplicate names, denominator drift, invalid statuses, inapplicable evidence, operation-contract drift, and per-capability metadata drift. Commands are hermetic and credential-free. Remaining 163 routes stay unresolved for Phases 4–7.
