...
#[test]
fn capability_logpush_discovery_example_is_exact() {
    let output = Command::new(env!("CARGO_BIN_EXE_magi-cloudflare-axi"))
        .args([
            "--format",
            "json",
...
#[test]
fn capability_autorag_discovery_example_is_exact() {
    let output = Command::new(env!("CARGO_BIN_EXE_magi-cloudflare-axi"))
        .args(["--format", "json", "capability", "get", "list_rags"])
        .output()
        .unwrap();
    assert!(output.status.success());
    let value: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
...
#[test]
fn capability_workers_builds_list_builds_discovery_example_is_exact() {
    let output = Command::new(env!("CARGO_BIN_EXE_magi-cloudflare-axi"))
        .args([
            "--format",
            "json",
